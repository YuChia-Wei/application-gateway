...
using application_gateway_lab.Infrastructure.Authentication.OpenIdConnect;
...
namespace application_gateway_lab.Infrastructure.Authentication;
...
public static class AuthenticationBuilderExtension
{
        ...
        builder.AddCookie(CookieAuthenticationDefaults.AuthenticationScheme, options =>
               {
                   ...
                   options.SessionStore =
                       new RedisCacheTicketStore($"{authOptions.LoginApplicationName}:LoginSession:",
                                                 authOptions.TicketStoreRedisServer);
                   ...
                   options.Events = new CookieAuthenticationEvents
                   {
                       OnValidatePrincipal = async cookieContext =>
                       {
                           var logger = cookieContext.HttpContext.RequestServices
                                                     .GetRequiredService<ILogger<CookieAuthenticationEvents>>();
                           ...
                           if (timeRemaining < refreshThreshold)
                           {
                               ...
                               var refreshToken =
                                   cookieContext.Properties.GetTokenValue(OpenIdConnectParameterNames.RefreshToken);
                               ...
                               var httpClient = cookieContext.HttpContext.RequestServices.GetRequiredService<IHttpClientFactory>()
                                                             .CreateClient();
    ...
    private static void AppendRoleToClaims(UserInformationReceivedContext context, JsonElement roleElement)
    {
        ...
        {
            claims.AddRange(
                roleElement.EnumerateArray().Select(r => new Claim(JwtClaimTypes.Role, r.GetString() ?? string.Empty)));
        }
        else
        {
